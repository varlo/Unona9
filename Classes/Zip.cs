...
namespace ionic.utils.zip
{
    ...
    public class ZipEntry
    {
        ...
        public delegate Stream FileParserDelegate(Stream file);

        public FileParserDelegate Parser
        {
            get { return _parser; }
            set { _parser = value; }
        }

        private FileParserDelegate _parser;
        ...
        private void WriteHeader(Stream s, byte[] bytes)
        {
            ...
            using (Stream input = File.OpenRead(FileName))
            {
                if (Parser != null)
                {
                    using (Stream parsedInput = Parser(input))
                    {
                        crc = crc32.GetCrc32AndCopy(parsedInput, CompressedStream);
                    }
                }
                else
                {
            ...
            // actual filename
            char[] c = InternalFileName.ToCharArray();
            int j = 0;
            ...
            _header = new byte[i];
            if (_Debug) Console.WriteLine("preserving header of {0} bytes", _header.Length);
            for (j = 0; j < i; j++)
                _header[j] = bytes[j];
        }
                           ...
                           IDisposable
    {
        ...
        private FileStream WriteStream
        {
            get
            {
                ...
                {
                    _writestream = new FileStream(_name, FileMode.CreateNew);
                }
                return _writestream;
            }
        }
        ...
        public void AddItem(string FileOrDirectoryName, bool WantVerbose)
        {
            if (File.Exists(FileOrDirectoryName))
                AddFile(FileOrDirectoryName, WantVerbose, null);
        ...
        {
            AddFile(FileName, false, null);
        }

        public void AddFile(string FileName, bool WantVerbose, ZipEntry.FileParserDelegate Parser)
        {
            ZipEntry ze = ZipEntry.Create(FileName);
            ze.InternalFileName = FileName.Replace(baseDir, String.Empty);
            ze.Parser = Parser;
